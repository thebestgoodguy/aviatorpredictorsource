...
public class Motion {
    ...
    public void buildPath(float[] points, int pointCount) {
        ...
        while (i < pointCount) {
            ...
            if (osc_x != null) {
                int r1 = i * 2;
                points[r1] = points[r1] + osc_x.get(position3);
            } else if (trans_x != null) {
                int r12 = i * 2;
                points[r12] = points[r12] + trans_x.get(position3);
            }
            if (osc_y != null) {
                int r13 = (i * 2) + 1;
                points[r13] = points[r13] + osc_y.get(position3);
            } else if (trans_y != null) {
                int r14 = (i * 2) + 1;
                points[r14] = points[r14] + trans_y.get(position3);
            }
            i++;
            f = 1.0f;
            motion = this;
    ...
    void buildBounds(float[] bounds, int pointCount) {
        ...
        while (i < r1) {
            ...
            motion.mStartMotionPath.getBounds(motion.mInterpolateVariables, motion.mInterpolateData, bounds, i * 2);
            i++;
            motion = this;
            r1 = pointCount;
            mils2 = mils;
    ...
    public void setup(int parentWidth, int parentHeight, float transitionDuration, long currentTime) {
        ...
        int r10 = this.mPathMotionArc;
        if (r10 != -1) {
            this.mStartMotionPath.mPathMotionArc = r10;
        }
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (!points[j].customAttributes.containsKey(attributeName) || (attribute = points[j].customAttributes.get(attributeName)) == null) {
                    ...
                    int[] r5 = this.mAttributeInterpolatorCount;
                    r5[i] = r5[i] + attribute.numberOfInterpolatedValues();
                    break;
                }
            }
            i++;
        ...
        int j2 = 0;
        while (true) {
            int[] r52 = this.mInterpolateVariables;
            if (j2 >= r52.length) {
                break;
            }
            int interpolateVariable = r52[j2];
    ...
    public boolean interpolate(MotionWidget child, float global_position, long time, KeyCache keyCache) {
        ...
        int r2 = this.mQuantizeMotionSteps;
        if (r2 == -1) {
            position = position2;
        } else {
            float steps = 1.0f / r2;
        ...
        if (curveFitArr != null) {
            ...
            if (this.mKeyTriggers != null) {
                ...
                while (true) {
                    ...
                    if (i2 >= motionKeyTriggerArr.length) {
                        break;
                    }
                    motionKeyTriggerArr[i2].conditionallyFire(position, child);
                    i2++;
                }
            }
        } else {
