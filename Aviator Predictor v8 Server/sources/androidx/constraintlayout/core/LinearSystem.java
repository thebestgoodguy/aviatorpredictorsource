...
public class LinearSystem {
    ...
    private SolverVariable acquireSolverVariable(SolverVariable.Type type, String prefix) {
        ...
        int r1 = this.mPoolVariablesCount;
        int r2 = POOL_SIZE;
        if (r1 >= r2) {
            int r22 = r2 * 2;
            POOL_SIZE = r22;
            this.mPoolVariables = (SolverVariable[]) Arrays.copyOf(this.mPoolVariables, r22);
        }
        SolverVariable[] solverVariableArr = this.mPoolVariables;
        int r23 = this.mPoolVariablesCount;
        this.mPoolVariablesCount = r23 + 1;
        solverVariableArr[r23] = variable;
        return variable;
    }
    ...
    private final void addRow(ArrayRow row) {
        ...
        if (SIMPLIFY_SYNONYMS && this.hasSimpleDefinition) {
            ...
            while (i < this.mNumRows) {
                ...
                ArrayRow[] arrayRowArr = this.mRows;
                if (arrayRowArr[i] != null && arrayRowArr[i].isSimpleDefinition) {
    ...
    public void removeRow(ArrayRow row) {
        ...
        if (row.isSimpleDefinition && row.variable != null) {
            if (row.variable.definitionId != -1) {
                int i = row.variable.definitionId;
                while (true) {
                    r1 = this.mNumRows;
                    if (i >= r1 - 1) {
                        break;
