...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        int r5 = this.parentHeight;
        int r4 = r5 - height;
        int r6 = this.insetTop;
        if (r4 < r6) {
            if (this.paddingTopSystemWindowInsets) {
                this.childHeight = r5;
            } else {
                this.childHeight = r5 - r6;
            }
        }
        this.fitToContentsOffset = Math.max(0, r5 - this.childHeight);
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed, int type) {
        ...
        if (dy > 0) {
            ...
            if (newTop > r4 && !this.hideable) {
                consumed[1] = currentTop - r4;
                ViewCompat.offsetTopAndBottom(child, -consumed[1]);
                setStateInternal(4);
            } else {
                if (!this.draggable) {
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && !this.callbacks.isEmpty()) {
            ...
            if (top > r1 || r1 == getExpandedOffset()) {
                int r12 = this.collapsedOffset;
                f = (r12 - top) / (this.parentHeight - r12);
            } else {
                int r13 = this.collapsedOffset;
                f = (r13 - top) / (r13 - getExpandedOffset());
            }
            float slideOffset = f;
            for (int i = 0; i < this.callbacks.size(); i++) {
    ...
    private void updateAccessibilityActions() {
        ...
        int r1 = this.expandHalfwayActionId;
        if (r1 != -1) {
            ViewCompat.removeAccessibilityAction(child, r1);
        }
